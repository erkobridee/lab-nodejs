// 04_mysqlTest_findAndUpdateRow.js
...
function selectTable(tablename, stop) {
  ...
  client.query(sql, function(err, rows) {
    if(err) {
      ...
      if(rows.length == 0) {
        ...
        if(stop) return endMysqlConnection(); 
        else return updateRow(tablename);
      }
    }
  });
};
...
function updateRow(tablename) { 
  ...
  //var id = 7;
  //findRowById(tablename, id, resultCallback);
  return findFisrtRow(tablename, resultCallback);
};
